 ...
 * <p>
 * A model can be rendered by creating a {@link com.badlogic.gdx.graphics.g3d.ModelInstance} from it. That instance has an additional transform to position the
 ...
 * <p>
 * A model is created from {@link NhgModelData}, which in turn is loaded by a {@link ModelLoader}.
 *
 * @author badlogic, xoppa, MovementSpeed
 */
public class NhgModel implements Disposable {
    /**
     ...
     **/
    public final Array<PBRMaterial> materials = new Array();
    /**
    ...
    /**
     * Constructs a new Model based on the {@link NhgModelData}. Texture files will be loaded from the internal file storage via an
     * {@link FileTextureProvider}.
     *
     * @param modelData the {@link NhgModelData} got from e.g. {@link ModelLoader}
     */
    public NhgModel(NhgModelData modelData) {
        this(modelData, new FileTextureProvider());
    }

    /**
     * Constructs a new Model based on the {@link NhgModelData}.
     *
     * @param modelData       the {@link NhgModelData} got from e.g. {@link ModelLoader}
     * @param textureProvider the {@link TextureProvider} to use for loading the textures
     */
    public NhgModel(NhgModelData modelData, TextureProvider textureProvider) {
        load(modelData, textureProvider);
    }

    protected void load(NhgModelData modelData, TextureProvider textureProvider) {
    ...
    protected void loadMaterials(Iterable<NhgModelMaterial> modelMaterials, TextureProvider textureProvider) {
        for (NhgModelMaterial mtl : modelMaterials) {
            this.materials.add(convertMaterial(mtl, textureProvider));
        }
    }

    protected PBRMaterial convertMaterial(NhgModelMaterial mtl, TextureProvider textureProvider) {
        ...
        if (mtl.textures != null) {
            for (NhgModelTexture tex : mtl.textures) {
                Texture texture;
