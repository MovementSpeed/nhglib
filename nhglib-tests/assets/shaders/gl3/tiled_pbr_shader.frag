...
#ifdef lights
    #if lights > 0
        //uniform LOWP sampler2D u_lightsTex;
        uniform LOWP vec2 u_countsAndOffsets[TILES_COUNT];
        uniform LOWP vec4 u_posAndRadiuses[MAX_LIGHTS];
        uniform LOWP vec4 u_colors[MAX_LIGHTS];
        //uniform LOWP int u_lightIndices[]
    #endif
#endif
...
vec3 calcLighting(int lightId, vec3 V, vec3 N, vec3 F0, vec3 albedo, float metalness, float roughness) {
    LOWP float lightRadius = u_posAndRadiuses[lightId].w;

    LOWP vec3 lightDirection = u_posAndRadiuses[lightId].xyz - v_position;
    ...
    LOWP vec3 radiance = u_colors[lightId].rgb;
    ...
    return (kD * albedo / M_PI + brdf) * radiance * NdotL * lightAttenuation;
}
...
void main() {
    ...
    #ifdef lights
        //LOWP ivec2 tilePos = ivec2(int(gl_FragCoord.x) / TILE_DIM, int(gl_FragCoord.y) / TILE_DIM);
        //int lightCount = int(u_countsAndOffsets[tilePos.x + tilePos.y * GRID_X].x);
        //int lightOffset = int(u_countsAndOffsets[tilePos.x + tilePos.y * GRID_X].y);

        //for (int i = 0; i < lightCount; ++i) {
            //int lightID = int(texture(u_lightsTex, vec2(float(lightOffset + i) / 64.0, 0.0)).x);
            //Lo += calcLighting(lightID, V, N, F0, albedo, metalness, roughness);
        //}
    #endif
