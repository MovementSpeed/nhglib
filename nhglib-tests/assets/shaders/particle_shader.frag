...
float stdDiff(float d, float softness) {
    return clamp((d) * softness, 0.0, 1.0);
}
...
void main() {
	...
	HIGH vec2 dcoords = vec2(gl_FragCoord.x / 1280.0, gl_FragCoord.y / 720.0);
	HIGH vec4 depthMap = texture2D(u_depthTexture, dcoords);

	HIGH float depth = depthMap.x;
	//HIGH float pdepth = length(v_position) / (4.0 - 0.01);
	HIGH float pdepth = (v_depth - 0.01) / (10.0 - 0.01);
    gl_FragDepth = pdepth;

	HIGH float zdiff = depth - pdepth;
