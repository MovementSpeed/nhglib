...
uniform mat4 u_viewMatrix;
        ...
        uniform int u_lightTypes[lights];
        uniform vec3 u_lightPositions[lights];
        uniform vec3 u_lightDirections[lights];
        uniform float u_lightIntensities[lights];
        uniform float u_lightInnerAngles[lights];
        uniform float u_lightOuterAngles[lights];
    #endif
#endif
...
void main() {
    ...
    F0 = mix(F0, albedo, metalness);
        ...
        for (int i = 0; i < int(ceil(pixel.r * 255.0)); i++) {
            ...
            vec3 lightDirection = u_lightPositions[lightId] - v_position;
            ...
            float NdotL = max(dot(N, L), 0.0) * u_lightIntensities[lightId];
